...
class LossHistory:
    ...
    def save(self):
        columns = [
            'index', 'epoch', 'sample_count', 'train_loss', 'val_loss',
            'epoch_val_loss'
        ]
...
def directed_hausdorff_3D(labels, preds):
    B,C,H,W=labels.shape
    labels = labels.reshape(B,C, -1)
    preds = preds.reshape(B,C, -1)
    return [directed_hausdorff(label, pred) for label,pred in zip(labels.cpu(),preds.cpu())[0]]
...
def process_files(input_dir, img_dir, meta_df, class_names):
    ...
    if os.path.exists(filename):
        ...
        # delete bad data
        df = df[~df['img_files'].str.contains('case7_day0')
                & ~df['img_files'].str.contains('case81_day30')].reset_index(
                    drop=True)
        return df

    # img_files = []
    # subdir = ''
    # # base_path=os.path.join(input_dir,img_dir)
    # # path=os.listdir(base_path)
    # while len(img_files) == 0:
    #     for img_path in path:
    #         img_files = sorted(glob(f'{base_path}/{img_path}/*.png'))
    #         subdir += '*/'
    #         if len(subdir) > 10:
    #             return None
    img_files = []
    ...
    while len(img_files) == 0:
        ...
        if len(subdir) > 10:
            return None
    mask_dir = '/home/ray/workspace/Fly_Pluche/kaggle/gi-tract/input/mask'
